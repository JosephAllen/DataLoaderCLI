...
IF "%JAVA_HOME%" == "" (
    for /f "tokens=*" %%i in ('dataloader-51.0.1-java-home.exe') do (
        IF EXIST "%%i" (
            set JAVA_HOME=%%i
        ) ELSE (
            echo %%i
...
IF "%JAVA_HOME%" == "" (
    ...
    IF NOT EXIST "%JAVA_HOME%" (
        ...
        "%JAVA_HOME%\bin\java" -cp ..\dataloader-51.0.1-uber.jar;..\lib\mssql-jdbc-9.2.0.jre11.jar -Dsalesforce.config.dir=%1 com.salesforce.dataloader.process.ProcessRunner %PROCESS_OPTION%
    )
)
